...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('입장설정')
    .setDescription('사용자의 입장 사운드와 메시지를 설정합니다.')
    ...
    .addStringOption(option => 
      option.setName('사운드')
        .setDescription('유튜브 링크 (입장 사운드)')
        .setRequired(true))
    .addStringOption(option => 
      option.setName('메시지')
        .setDescription('입장 메시지')
        .setRequired(false)),
  ...
  async execute(interaction) {
    ...
    // 입장 설정 저장
    userSettings[targetUser.id][interaction.guild.id].enterSound = sound;
    userSettings[targetUser.id][interaction.guild.id].enterMessage = message;
    
    // 파일에 저장
    fs.writeFileSync(settingsPath, JSON.stringify(userSettings, null, 2));
    
    await interaction.reply({ 
      content: `${targetUser.username}의 입장 설정이 완료되었습니다.\n사운드: ${sound}${message ? `\n메시지: ${message}` : ''}`, 
      ephemeral: true 
    });
  },
}; 